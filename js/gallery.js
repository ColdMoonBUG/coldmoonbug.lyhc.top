...
// —— 2. 读取 URL 参数：cat（分类），page（当前页）
const params = new URLSearchParams(window.location.search);
const cat    = params.get("cat") || "group1";
const page   = parseInt(params.get("page"), 10) || categories[cat].pages[0].num;

// —— 3. 获取本分类的页码数组
const currentCategory = categories[cat] || categories.group1;
const nums             = currentCategory.pages.map(p => p.num);
